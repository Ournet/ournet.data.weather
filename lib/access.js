...
Service.prototype.getForecast = function(selector) {
	return db.access.getItem('WeatherForecast', selector.key, {
			format: 'json'
		})
		.then(function(report) {
			...
			return forecast.get(selector)
				.then(function(newreport) {
					...
					logger.error('Eror on getting forecast', error);
					return null;
				});
		});
};
...
Service.prototype.getAlarms = function(key, options) {
	return db.access.query('WeatherAlarm', key,
		_.defaults({}, options || {}, { format: 'json', key: key }));
};

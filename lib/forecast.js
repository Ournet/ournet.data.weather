...
forecast.formatSelector = function(place) {
	return {
		...
		region: helpers.weatherAdmName(place),
	...
	return new Promise(function(resolve, reject) {
		request({
			...
			if (error || response.statusCode >= 400) {
				return reject(error || new Error(response.message || 'Error on getting weather report'));
			}
...
internal.formatReport = function(data) {
	...
	data.forEach(function(time) {
		...
		// convert to UTC unix time:
		item.from = Date.parse(item.from) - offset;
		item.to = Date.parse(item.to) - offset;

		cday = new Date(item.from + offset);
		...
		item.wind.speed.beaufort = helpers.windSpeedToBeaufort(item.wind.speed.mps);
